...
u32 get_format_size(const s32 format)
{
	// TODO(velocity)
	return 4;
}
...
u32 get_video_buffer_size(const CellCameraInfoEx& info)
{
	...
	return width * height * get_format_size(info.format);
}
...
s32 cellCameraOpenEx(s32 dev_num, vm::ptr<CellCameraInfoEx> info)
{
	...
	}
	{
		...
		const auto vbuf_size = get_video_buffer_size(g_camera->info);

		g_camera->image_data_ptr = vm::cast(vm::alloc(vbuf_size, vm::memory_location_t::main));
		info->buffer = g_camera->image_data_ptr;
	}
	return CELL_OK;
}
...
s32 cellCameraClose(s32 dev_num)
{
	...
	}
	{
		...
		vm::dealloc(g_camera->image_data_ptr.addr(), vm::memory_location_t::main);
...
s32 cellCameraIsAvailable(s32 dev_num)
{
	...
	auto ret = cellCameraGetType(dev_num, type);
	if (ret != CELL_OK || !*type)
		return false;

	vm::var<u32> arg;

	auto ret2 = cellCameraGetAttribute(dev_num, CELL_CAMERA_DEVICESPEED, arg, vm::null);
	if (!*arg)
		return false;
...
s32 cellCameraReadEx(s32 dev_num, vm::ptr<CellCameraReadEx> read)
{
	...
	auto shared_data = fxm::make_always<gem_camera_shared>();
...
{
	cellCamera.notice("cellCameraReadComplete(dev_num=%d, bufnum=%d, arg2=%d)", dev_num, bufnum, arg2);
		...
		for (auto const& notify_data_entry : notify_data_map)
		{
				...
				info.framerate != 0)
			{
				if (auto queue = lv2_event_queue::find(key))
				{
					...
					{
						const u32 image_data_size = info.bytesize;
						const u16 buffer_number = 0;
						const u16 camera_id = 0;
					...
					default:
					{
						cellCamera.error("Unknown read mode set: %d. This should never happen.", read_mode);
						return;
					}
					}
